...
void BLER_CURVE::Create_CURVE(FILE *stream,int *curve_num,double **sinr_data,double **bler_data)
{
	...
	else
	{
		for(int k=0;k<MCS_max;k++)
		{
			...
			{
				fscanf( stream, "%lg ",&sinr_data[k][i]);
			}

			for(int i=0; i<curve_num[k];i++)
			{
				fscanf( stream, "%lg ",&bler_data[k][i]);
			}
		}
	}
